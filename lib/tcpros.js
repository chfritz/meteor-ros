var net           = Npm.require('net')
  ...
  , portscanner   = Npm.require('portscanner')
...
TCPROS.prototype.callService = function(
  port, host, service, message, serviceType) {
  
  var socket = net.createConnection(port, host)
  socket.on('data', function(caller_id) {
    console.log("service response:", caller_id.toString('ascii'));
  });
...
function serializeMessage(message) {
  var bufferSize   = fields.getMessageSize(message)
...
function serializeInnerMessage(message, buffer, bufferOffset) {
  message.fields.forEach(function(field) {
    ...
    if (fields.isPrimitive(field.type)) {
      fields.serializePrimitive(field.type, fieldValue, buffer, bufferOffset);
      console.log(buffer.toString('hex'));
      bufferOffset += fields.getPrimitiveSize(field.type, fieldValue);
    }
    else if (fields.isArray(field.type)) {
      buffer.writeUInt32LE(fieldValue.length, bufferOffset);
      bufferOffset += 4;

      var arrayType = fields.getTypeOfArray(field.type);
      fieldValue.forEach(function(value) {
        if (fields.isPrimitive(arrayType)) {
          fields.serializePrimitive(arrayType, value, buffer, bufferOffset);
          bufferOffset += fields.getPrimitiveSize(arrayType, value);
        }
        else if (fields.isMessage(arrayType)) {
          serializeInnerMessage(value, buffer, bufferOffset);
          bufferOffset += fields.getMessageSize(value)
        }
      });
    }
    else if (fields.isMessage(field.type)) {
      serializeInnerMessage(fieldValue, buffer, bufferOffset)
      bufferOffset += fields.getMessageSize(fieldValue)
    }
  });
}
...
function deserializeInnerMessage(message, buffer, bufferOffset) {
  message.fields.forEach(function(field) {
    ...
    if (fields.isPrimitive(field.type)) {
      fieldValue = fields.deserializePrimitive(field.type, buffer, bufferOffset)
      bufferOffset += fields.getPrimitiveSize(field.type, fieldValue)
    }
    else if (fields.isArray(field.type)) {
      var array     = []
        , arraySize = buffer.readUInt32LE(bufferOffset)
        , arrayType = fields.getTypeOfArray(field.type)
        ;
      bufferOffset += 4;

      for (var i = 0; i < arraySize; i++) {
        if (fields.isPrimitive(arrayType)) {
          var value = fields.deserializePrimitive(arrayType, buffer, bufferOffset);
          bufferOffset += fields.getPrimitiveSize(arrayType, value);
          array.push(value);
        }
        else if (fields.isMessageType(arrayType)) {
          var arrayMessage = new field.messageType();
          arrayMessage = deserializeInnerMessage(arrayMessage, buffer, bufferOffset);
          bufferOffset += fields.getMessageSize(arrayMessage);
